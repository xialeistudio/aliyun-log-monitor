...
function download(path, ossOptions) {
	...
	co(function*() {
		...
		if (result.objects === undefined) {
			console.info('[oss] list empty');
			ee.emit('listEmpty');
		}
		else {
			console.info('[oss] need download %d objects', result.objects.length);
			...
			ee.on('_download', function(index) {
				...
				co(function*() {
					console.info('[oss] begin download %s',object.name);
					yield client.get(object.name, path + '/' + getBaseName(object.name));
					downloaded++;
					console.info('[oss] %d/%d: %s download success', downloaded, total, object.name);
					//触发外部事件
					...
					downloaded++;
					console.error('[oss] %d/%d: %s download error', downloaded, total, object.name);
